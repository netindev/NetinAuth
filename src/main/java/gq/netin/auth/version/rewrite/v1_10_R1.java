 ...
 *
 */
public class v1_10_R1 extends LoginListener {

	private static final MinecraftServer SERVER = ((CraftServer) Bukkit.getPlugin().getServer()).getServer();

	public v1_10_R1(NetworkManager networkManager, String player) {
		super(v1_10_R1.SERVER, networkManager);
	...
	public void E_() {
		...
		EntityPlayer attemptLogin = v1_10_R1.SERVER.getPlayerList().attemptLogin(this, validProfile, this.hostname);
		if (attemptLogin != null) {
			this.networkManager.sendPacket(new PacketLoginOutSuccess(validProfile));
			v1_10_R1.SERVER.getPlayerList().a(this.networkManager,
					v1_10_R1.SERVER.getPlayerList().processLogin(validProfile, attemptLogin));
		}
	}
